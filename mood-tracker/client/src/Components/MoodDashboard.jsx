  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(`https://full-stack-projects-yvmx.onrender.com/journal/mood-data`, {
          headers: {
        ...
        if (transformedData.length >= 7) {
          ...
          const freqResponse = await fetch('https://full-stack-projects-yvmx.onrender.com/journal/mood-frequency', {
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
          });
